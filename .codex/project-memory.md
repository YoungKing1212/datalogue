...
- 修复数据集页面顶部能力卡“数据表”计数、LeadAgent 两阶段 Planner Prompt 重复说明并同步 Langfuse production v4、新建对话刷新后固定进入最近对话顶部；验证覆盖前端单测/lint/build、后端会话测试和相关观测测试。
...
- 关键改动：按 DAT-11 要求先保存开发计划；新增 `AskBIRequest/AskBIResponse`、`DatalogueEventEnvelope`、`ArtifactCard`、refs 和 `ask_bi` 最小契约；新增 AgentScope Shell Adapter service，第一阶段固定只允许 `ask_bi`，不开放公开 API、不接前端、不启动 runner；新增 AgentScope Event Adapter，`control_plane` 事件只计入内部丢弃数，不进入 Shell 可见事件或 trace 事件输出。
- 验证方式：执行 `cd datalogue-api && python3 -m pytest tests/test_agentscope_shell_adapter.py tests/test_agentscope_event_adapter.py tests/test_bi_workbench_tool.py tests/test_event_envelope.py tests/test_subagent_tool_adapter.py -q`，19 条用例通过；执行 `cd datalogue-api && python3 -m py_compile app/schemas/bi_workbench.py app/services/bi_workbench_tool.py app/services/agentscope_shell_adapter.py app/services/agentscope_event_adapter.py` 通过；执行 `git diff --check -- datalogue-api/app/schemas/bi_workbench.py datalogue-api/app/services/bi_workbench_tool.py datalogue-api/app/services/agentscope_shell_adapter.py datalogue-api/app/services/agentscope_event_adapter.py datalogue-api/tests/test_agentscope_shell_adapter.py datalogue-api/tests/test_agentscope_event_adapter.py datalogue-api/tests/test_bi_workbench_tool.py datalogue-api/tests/test_event_envelope.py .omx/plans/DAT-11-agentscope-shell-adapter.md` 通过。
- 残留风险：当前是 contract-first 最小验证线，未接真实 `/chat/stream`、未导入 AgentScope runtime、未做真实 BI 主链回放；后续 P0.5/P0.6/P1.1 完整落地后可把 `BIWorkbenchTool` 的 handler 接到真实主链。
