...
- 修正数据集页面“数据表”能力卡计数口径为已选表数量，并用组件测试、lint、build 验证。
- 压缩 LeadAgent 两阶段 Planner Prompt 重复规则并同步 Langfuse production v4，保持 JSON 契约不变。
...
### 2026-06-26 17:10 · QueryGraph Compiler 与 SQL 方言适配外壳
...
- 关键改动：新增 QueryPlan 工具编译器与方言适配外壳，编译产物统一输出 `execution_source=tool_compiler`；当前阶段只允许 `sqlite/mysql` 真实方言，未知方言 fail closed；compiler context 显式剔除 `llm_sql/direct_sql/raw_sql/sql` 等 SQL 文本字段；SubAgent 初始态写入 `query_plan_compilation`、`control_plane`、`query_artifact`，Graph 在编译产物可用时跳过 DSL LLM SQL 生成，执行层只透传工具编译 SQL。
- 验证方式：执行 `cd datalogue-api && pytest tests/test_query_plan_compiler.py tests/test_sql_dialect_adapter.py tests/test_subagent_run.py -q`，21 条用例通过；执行 `cd datalogue-api && pytest tests/test_subagent_planning_contracts.py tests/test_dataset_prompt_instructions.py -q`，37 条用例通过。
