...
### 2026-06-28 12:32 · C1 RepairPlan 契约、事件与前端承接

- 涉及文件：`datalogue-api/app/schemas/repair_plan.py`、`datalogue-api/app/services/repair_plan.py`、`datalogue-api/app/schemas/bi_workbench.py`、`datalogue-api/app/services/artifact_store.py`、`datalogue-api/app/api/artifacts.py`、`datalogue-api/app/api/chat.py`、`datalogue-api/app/graph/nodes.py`、`datalogue-api/tests/test_repair_plan_contract.py`、`datalogue-api/tests/test_bi_main_chain_acceptance.py`、`datalogue-api/tests/test_sql_audit.py`、`datalogue-web/src/assistant/chat-adapter.js`、`datalogue-web/src/assistant/MyMessage.jsx`、`datalogue-web/src/components/artifact-card.jsx`、`docs/main-chain-acceptance-records/2026-06-28-c1-repair-plan-acceptance.md`、`docs/architecture/C1-RepairPlan-真实成功链路设计.md`、`docs/superpowers/specs/2026-06-28-c1-repair-plan-real-acceptance-design.md`
- 关键改动：新增 RepairPlan v1 schema 和服务层分类/重跑次数/Tool 校验/脱敏摘要；扩展 `repair.*` event envelope、`ArtifactKind=repair_plan`、`ArtifactRef.ref_type=repair_plan`，统一使用 `artifact:<uuid>` 句柄；Artifact API 对 repair_plan 只返回脱敏摘要；`/chat/stream` 在 SQL audit 后持久化 RepairPlan artifact、发出 `repair.evaluated/plan_created/rerun_started/rerun_completed/blocked` 事件，并把 repair refs 写入 final payload、ArtifactCard related_refs、trace metadata 和 conversation_state facts；SQL audit 在 RepairPlan 校验失败时同步阻断 workflow retry；前端 chat-adapter、MyMessage 和 ArtifactCard 增加业务级 repair timeline/确认卡/refs 渲染并过滤 patch、schema、SQL、raw result；设计文档修正 C1 不做管理员详情 UI、Langfuse 验收分层和 `repair_plan_ref` 前缀契约。
- 验证方式：执行 `cd datalogue-api && python3 -m pytest tests/test_repair_plan_contract.py tests/test_event_envelope.py tests/test_artifact_card_contract.py tests/test_artifact_api.py tests/test_agentscope_event_adapter.py tests/test_sql_audit.py tests/test_bi_main_chain_acceptance.py tests/test_chat.py tests/test_observability.py -q`，195 条通过；执行 `cd datalogue-api && python3 -m pytest tests/test_bi_main_chain_acceptance.py::test_repair_plan_success_cross_checks_five_evidence_sets tests/test_bi_main_chain_acceptance.py::test_repair_plan_blocked_emits_repair_events_without_artifact_ref -q`，2 条通过；执行 `cd datalogue-web && npm run test -- src/assistant/chat-adapter.test.js src/assistant/MyMessage.test.jsx src/components/artifact-card.test.jsx`，3 个文件 32 条通过；执行 `cd datalogue-web && npm run lint` 通过，保留既有 15 个 warning；执行 `cd datalogue-web && npm run build` 通过，仅保留既有 Vite chunk warning；执行 `cd datalogue-api && python3 -m py_compile app/schemas/repair_plan.py app/services/repair_plan.py app/schemas/bi_workbench.py app/services/artifact_store.py app/api/artifacts.py app/api/chat.py app/graph/nodes.py app/schemas/__init__.py` 通过；本地 API 用主工作区 `.venv` 启动后 `import langfuse` 为 4.7.1，`GET /health` 通过。
- 残留风险：真实问题 `查询杨凯 2024 年工作日志` 尚未成功查数；一次真实请求进入 `FIELD_NOT_FOUND/blocked`，没有生成 `repair_plan_ref`，另一次提前停在蓝图参数 `person_name` 缺失；当前 RepairPlan 尚未实现字段级 QueryGraph patch Tool 引擎，仍依赖 `sql_audit -> increment_retry -> dsl_generate` 旧重试链；真实 Langfuse UI observation 未手工核对；失败查询仍可能写入 `last_success_task`，需要后续单独修复。

### 2026-06-28 13:35 · C1 真实问题剩余故障收口

- 涉及文件：`datalogue-api/app/services/subagent_planning/contracts.py`、`datalogue-api/app/services/subagent_planning/planner.py`、`datalogue-api/app/services/dataset_subagent.py`、`datalogue-api/app/api/chat.py`、`datalogue-api/app/graph/nodes.py`、`datalogue-api/tests/test_subagent_planning_contracts.py`、`datalogue-api/tests/test_subagent_execution.py`、`datalogue-api/tests/test_subagent_query_planner.py`、`datalogue-api/tests/test_chat.py`、`.codex/project-memory.md`
- 关键改动：修复真实问题 `查询杨凯 2024 年工作日志` 的剩余阻断点：`CandidateAsset.usage` 兼容 `template_reference/reference_only` 别名；Dataset 10 工作日志模板规划在 `planner_source=template/deterministic` 且参数完整时跳过 LLM 改写，避免被重写成宽字段 QueryGraph；蓝图执行参数从 route payload、QueryPlan required inputs、entities 和自然语言问题中恢复 `person_name`；`dsl_generate_node` 优先使用可信模板 SQL，再考虑 QueryPlan 工具编译结果，避免模板 SQL 被宽字段编译器覆盖；`_persist_completed_turn` 增加 final payload 成功门禁，澄清、拒答、Repair blocked/failed、SQL diagnosis、蓝图失败等不再污染 `last_success_task`。
- 验证方式：按 TDD 先新增红灯用例，分别复现 usage alias 拒绝、蓝图参数缺失、失败 final 写入 last_success、模板规划仍调用 LLM、模板 SQL 被 tool compiler 覆盖；实现后执行 `cd datalogue-api && .venv/bin/python -m pytest tests/test_repair_plan_contract.py tests/test_sql_audit.py tests/test_event_envelope.py tests/test_artifact_card_contract.py tests/test_artifact_api.py tests/test_agentscope_event_adapter.py tests/test_bi_main_chain_acceptance.py tests/test_subagent_planning_contracts.py tests/test_subagent_execution.py tests/test_subagent_query_planner.py tests/test_chat.py -q`，249 条通过、3 条跳过；执行 `cd datalogue-web && npm run test -- src/assistant/chat-adapter.test.js src/assistant/MyMessage.test.jsx src/components/artifact-card.test.jsx && npm run lint && npm run build`，32 条前端用例通过、lint 0 error 15 warning、build 通过；执行 `python -m py_compile` 覆盖本次修改的 5 个后端文件；执行 `git diff --check` 通过。
- 真实验收：本地 API 以 `127.0.0.1:8001` 启动后，对 `/api/chat/stream` 发送 `{"question":"查询杨凯 2024 年工作日志","dataset_id":10,"session_id":"c1-repair-live-20260628-v5"}` 成功返回 21 个 SSE 事件；`conversation_id=23`、`message_id=52`、`task_id=conv-23-msg-52`、`trace_id=27fea2554d0e4dd3389565f37c4fa5df`；回答为杨凯 2024 年工作日志 100 条；`result_ref=artifact:4fab8a78d546480699ac6a5e1c35485e`、`report_ref=artifact:56e7e6b33a724f158cf8c73cbc2a1ff6`；Artifact API 可读主查询产物和报告产物，主产物 `row_count=100` 且首行姓名为杨凯；后端日志显示 `generation_mode=template`、`SQL执行成功: 返回 100 行`、`last_success_task` 写入 ready。
- 残留风险：这次真实问题最终通过可信模板 SQL 直接成功，未触发 RepairPlan rerun；RepairPlan 自动修复链路仍以自动化 fixture 验证为主，真实 Langfuse UI observation 仍需手工核对并补充外部截图或记录。

### 2026-06-28 14:25 · C1 页面 E2E 与 Chat SQL 可见层收口

- 涉及文件：`datalogue-web/src/assistant/chat-adapter.js`、`datalogue-web/src/assistant/thread-list-adapter.js`、`datalogue-web/src/assistant/MyMessage.jsx`、`datalogue-web/src/assistant/Thread.jsx`、`datalogue-web/src/components/chat-page.jsx`、`datalogue-web/src/components/agent-panel.jsx`、`datalogue-web/src/App.jsx`、`datalogue-web/src/assistant/chat-adapter.test.js`、`datalogue-web/src/assistant/MyMessage.test.jsx`、`.codex/project-memory.md`
- 关键改动：基于真实页面 E2E 发现普通 Chat 可见层仍展示 `dsl_compiler` SQL 片段和 `SQL 复制` 按钮，修复为：reasoning 只显示“查询语句已生成并进入执行校验”；`metadata.custom.sql` 固定为空；历史回放不再从 message metadata 或 step_trace 恢复 SQL；前端用户可见 trace 递归删除 `sql/raw_sql/direct_sql/llm_sql/compiled_sql/sql_list`；Chat 气泡和 AgentPanel 删除 SQL 折叠块、复制入口和 Tweaks 中“显示生成的 SQL”开关；AgentPanel 结果摘要不再展示字段名列表。
- 验证方式：执行 `cd datalogue-web && npm run test -- src/assistant/chat-adapter.test.js src/assistant/MyMessage.test.jsx src/components/artifact-card.test.jsx`，34 条用例通过；执行 `cd datalogue-web && npm run lint`，0 error、15 个既有 warning；执行 `cd datalogue-web && npm run build` 通过，仅保留既有 chunk size warning；执行 `git diff --check` 通过。
- 页面 E2E：隔离启动 worktree API `127.0.0.1:8001`、worktree Vite `127.0.0.1:5174`，并用临时代理 `127.0.0.1:5180` 路由页面和 API；Browser 打开 `/chat`，发送 `查询杨凯 2024 年工作日志`，先出现候选数据集确认，点击 `生产经营管理系统日志数据集` 后同一会话 `/chat/25` 成功返回 100 条；页面展示 `BI 查询结果` ArtifactCard，`artifact:75471820c0e24c8a860b7400c48d9598`、`artifact:4c65834e083e43aa91904ec5132e5574`、`trace:78b264b6861d190567d5ed4f60fd28ff`、`checkpoint://conv-25-msg-60/query_context_ready` 一致可见；DOM 检查 `SELECT`、`SQL 复制`、`复制 SQL`、`显示生成的 SQL` 均为 false；Artifact API 验证主查询产物 `conversation_id=25/message_id=60/kind=sql_result/person_name=杨凯`，报告产物 `kind=report` 可读。
- 残留风险：Browser 内置截图接口 `Page.captureScreenshot` 连续超时，已用 `npx playwright screenshot http://127.0.0.1:5180/chat/25 /tmp/datalogue-c1-e2e-chat25.png` 作为视觉证据 fallback；Browser 日志接口仍返回热更新过程中出现过的旧 `showSql/sqlText` ReferenceError，但源码 `rg`、生产 build 和新页面 DOM 均确认该引用已清除，当前页面可正常渲染。

### 2026-06-28 15:05 · C1 页面会话切换 E2E 收口

- 涉及文件：`datalogue-web/src/assistant/ThreadList.jsx`、`datalogue-web/src/components/chat-page.jsx`、`datalogue-web/src/components/chat-page.test.jsx`、`datalogue-web/tests/unit/assistant/thread-list-new-conversation.test.jsx`、`.codex/project-memory.md`
- 关键改动：基于页面 E2E 修复会话切换三类问题：点击侧栏历史会话后消息区已切换但 URL 仍停在旧 `/chat/:id`；直接打开 `/chat/24` 时被初始 runtime 默认 main thread 回滚到 `/chat/21`；点击“新对话”后 `/chat` 又被旧 remoteId 自动补回历史会话。新增 `resolveUrlSyncTarget()`，只有 runtime 主线程真实变更时才反向同步 URL，首次深链和 route-driven pending 状态不回滚；历史会话按钮增加 `data-conversation-id` 和包含会话号的 `aria-label`，用于重复标题下的精确访问和 E2E 定位。
