...
- 修正数据集页面顶部能力卡“数据表”计数为已选表数量；精简并同步 LeadAgent 两阶段 Planner Prompt 到 Langfuse；修复新建对话进入最近对话顶部和后端会话列表稳定排序。
...
- 关键改动：新增 DAT-13 计划文件；`dataset_router` 内部仍用 Manifest capability 摘要打分，但对外候选只暴露 `dataset_id/dataset_name/reason/confidence/requires_confirmation`；低置信和多数据集近分均标记需确认且不 dispatch；`lead_agent_routing` 在未确认数据集时阻断指标/明细问法直接进入 `query_graph`；`chat` 阻断提示改读 `confidence/reason`。
