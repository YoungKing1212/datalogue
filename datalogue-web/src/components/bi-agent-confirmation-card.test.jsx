...
import BIAgentConfirmationCard, {
  BIAgentConfirmationCard as NamedBIAgentConfirmationCard,
} from './bi-agent-confirmation-card.jsx';
...
describe('BIAgentConfirmationCard', () => {
  it('renders route-level capability summary without sensitive fields', () => {
    render(<BIAgentConfirmationCard run={confirmationRun} onConfirm={vi.fn()} />);

    expect(screen.getByText('BI Agent')).toBeInTheDocument();
  ...
  it('confirms with a trimmed capability snapshot and approved decision', () => {
    ...
    render(<NamedBIAgentConfirmationCard run={confirmationRun} onConfirm={onConfirm} />);
  ...
  it('renders null when the run or dataset id is missing', () => {
    const { container, rerender } = render(<BIAgentConfirmationCard run={null} />);
    expect(container.firstChild).toBeNull();

    rerender(<BIAgentConfirmationCard run={{ confirmation_request: {} }} />);
    expect(container.firstChild).toBeNull();
  });
});
