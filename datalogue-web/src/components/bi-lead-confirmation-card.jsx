// bi-lead-confirmation-card.jsx
// BI LeadAgent 查询范围确认卡片：只展示路由级能力摘要，确认时仅回传可审计的白名单字段。
...
function Field({ label, value }) {
  ...
  return (
    <div className="bi-lead-confirmation-card__field">
      <dt>{label}</dt>
      <dd>{String(value)}</dd>
    </div>
  );
...
function TagList({ label, items }) {
  ...
  return (
    <div className="bi-lead-confirmation-card__section">
      <span className="bi-lead-confirmation-card__section-label">{label}</span>
      <ul className="bi-lead-confirmation-card__tags" aria-label={label}>
...
export function BILeadConfirmationCard({ run, onCancel, onConfirm }) {
  ...
  return (
    <section className="bi-lead-confirmation-card" aria-label="BI LeadAgent 确认查询范围">
      <header className="bi-lead-confirmation-card__header">
        <div>
          <p className="bi-lead-confirmation-card__eyebrow">BI LeadAgent</p>
          <h3>确认查询范围</h3>
        </div>
      </header>

      <dl className="bi-lead-confirmation-card__summary">
      ...
      {request.routing_rationale && (
        <div className="bi-lead-confirmation-card__notice">
          <span>选择理由</span>
      ...
      {request.risk_notice && (
        <div className="bi-lead-confirmation-card__notice bi-lead-confirmation-card__notice--risk">
          <span>提示</span>
          <p>{request.risk_notice}</p>
        </div>
      )}

      <footer className="bi-lead-confirmation-card__actions">
