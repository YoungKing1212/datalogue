...
import BILeadAgentFlow, { buildConfirmationRequest } from './bi-lead-agent-flow.jsx';
...
describe('BILeadAgentFlow', () => {
  it('builds a route-level confirmation request without execution internals', () => {
    ...
    expect(request).toEqual({
      ...
      routing_rationale: '用户已选择 销售订单明细，BI LeadAgent 将把查询任务交接给 DatasetAgent。',
  ...
  it('disables run creation until a dataset is selected', () => {
    render(<BILeadAgentFlow initialQuestion="统计 GMV" selectedDataset={null} />);

    expect(screen.getByRole('button', { name: '创建 run' })).toBeDisabled();
    expect(screen.getByText('请选择一个数据集后再启动 BI LeadAgent。')).toBeInTheDocument();
  });
