...
import BIAgentRunPanel, { BIAgentRunPanel as NamedBIAgentRunPanel } from './bi-agent-run-panel.jsx';

describe('BIAgentRunPanel', () => {
  ...
  it('falls back for completed runs when answer summary only contains result internals', () => {
    ...
    expect(screen.getByText('BI Agent 正在处理本次查询。')).toBeInTheDocument();
  ...
  it('renders null without a run', () => {
    const { container } = render(<BIAgentRunPanel run={null} />);
