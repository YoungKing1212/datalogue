...
| 验收面 | 证据 | 结果 |
| --- | --- | --- |
| API 主链路 / 观测 / Artifact | `cd datalogue-api && python3 -m pytest tests/test_bi_main_chain_acceptance.py tests/test_chat.py tests/test_observability.py tests/test_artifact_api.py -q` | 141 passed |
| 前端 Chat metadata | `cd datalogue-web && npm run test -- src/assistant/chat-adapter.test.js` | 6 passed |
...
| 前端 ArtifactCard / 历史回放 | `cd datalogue-web && npm run test -- src/components/artifact-card.test.jsx tests/unit/assistant/artifact-custom.test.js src/assistant/chat-adapter.test.js` | 26 passed |
| 前端质量门禁 | `cd datalogue-web && npm run lint`；`cd datalogue-web && npm run build` | 通过，保留既有 15 个 lint warning 和 Vite chunk warning |
| 空白检查 | `git diff --check` | 通过 |
| 最终集成套件 | `cd datalogue-api && python3 -m pytest tests/test_capability_manifest.py tests/test_bi_soul_contract.py tests/test_lead_agent_capability_router.py tests/test_query_plan_compiler.py tests/test_sql_dialect_adapter.py tests/test_subagent_tool_adapter.py tests/test_event_envelope.py tests/test_bi_workbench_tool.py tests/test_agentscope_shell_adapter.py tests/test_agentscope_event_adapter.py tests/test_artifact_card_contract.py tests/test_retry_checkpoint.py tests/test_legacy_conversation_replay.py tests/test_bi_main_chain_acceptance.py tests/test_chat.py -q`；`cd datalogue-web && npm run test` | 后端 168 passed；前端 9 个文件 62 passed |
