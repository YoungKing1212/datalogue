...
class BIHandoffService:
    """BI LeadAgent handoff 应用服务；确认门禁通过后才允许调用 DatasetAgent Runtime。"""
    ...
    async def query_dataset(self, *, run_id: int) -> BILeadAgentHandoffResult:
    ...
    def _persist_handoff(self, *, run: BILeadAgentRun, result: BILeadAgentHandoffResult) -> BIAgentHandoff:
        ...
        self.db.add(handoff)  # 只落 BILeadAgentHandoffResult 白名单字段，避免 DatasetAgent 内部执行态写入 handoff 表。
        return handoff

    def _complete_run_from_handoff(self, *, run: BILeadAgentRun, result: BILeadAgentHandoffResult) -> None:
