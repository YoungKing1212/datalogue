...
# File Name   : atomic.py
# Description:
#   AgentScope 2.0 形态的 BI 原子工具实现。
#
# Responsibilities:
#   - 用 ToolBase 子类表达 Datalogue 的最小 BI 原子工具。
#   - 用 Toolkit 作为 DatasetAgent Runtime 可见的工具容器。
...
class BIAtomicToolContext:
    ...
    sanitizer: DatalogueAgenticShell = field(default_factory=DatalogueAgenticShell)
    ...
    def execute_external(
        ...
        *,
        ...
        try:
            ...
            execution_result = _normalize_execution_result(self.context.query_executor(str(compiled["sql"])))
            ...
            if failure.get("code") == "FIELD_NOT_FOUND":
                compiled["last_execution_failure"] = {
                    ...
                    "missing_column_ref": _extract_missing_column_ref(exc),
                }
            return failure
...
class DatalogueBIAtomicToolkit:
    """DatasetAgent Runtime 使用的 BI 工具容器。"""
...
def build_bi_atomic_toolkit(
    ...
    *,
    ...
    """构建 AS-R0 DatasetAgent Runtime 可见的 AgentScope Toolkit。"""
    ...
    for table_schema in sql_generation_context.get("table_schemas") or []:
        ...
        for field in table_schema.get("fields") or []:
            ...
            if not column_name or column_name == old_column:
                continue
            field_label = _normalize_business_label(str(field.get("display_name") or field.get("name") or ""))
