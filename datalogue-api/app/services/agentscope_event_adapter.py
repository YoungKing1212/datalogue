...
class AgentScopeEventAdapter:
    ...
    def map_events(
        ...
        for item in envelopes:
            ...
            visibility = (
                envelope.visibility
                if isinstance(envelope.visibility, EventVisibility)
                else EventVisibility(envelope.visibility)
            )
            if visibility == EventVisibility.CONTROL_PLANE:
            ...
            shell_event = AgentScopeShellEvent(
                ...
                channel=(
                    "shell_visible"
                    if visibility == EventVisibility.USER_VISIBLE
                    else "trace"
                ),
                payload=sanitize_outer_payload(envelope.payload),  # trace 也只暴露验证摘要，避免旧 payload 夹带敏感键。
