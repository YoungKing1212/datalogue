...
# Description:
#   BI Agent 到 Dataset Query Skill 的 Host Handoff Adapter。
#
# Responsibilities:
#   - 将已确认的数据集任务交给 AgentScope Dataset Query Skill 执行。
...
class DatalogueBIHandoffAdapter:
    ...
    async def query_dataset(
        ...
        )  # session 是 Dataset Query Skill 的私有执行态，返回结果只暴露 refs 和摘要。
        ...
        try:
            ...
            logger.exception("BI Agent handoff failed while running AgentScope DatasetAgent.")
            return BIAgentHandoffResult(
                handoff_id=handoff_id,
                child_run_id=child_run_id,
                dataset_id=request.dataset_id,
