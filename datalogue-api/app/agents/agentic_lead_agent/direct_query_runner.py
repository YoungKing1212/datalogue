...
class AgenticDirectQueryRunner:
    ...
    async def stream(
        ...
        *,
        ...
        # 发射 agent.handoff 事件：LeadAgent → BI Agent
        yield {
    ...
    async def _run_bi_agent_tool_loop(
        ...
        *,
        ...
        for turn_index in range(MAX_BI_AGENT_TOOL_TURNS):
            ...
            # emit tool_call 事件（从 bridge on_tool_call 收集并 yield）
            tool_events: list[dict[str, Any]] = []
            def _collect_tool_event(event_type, tool_name, tool_call_id, payload):
                tool_events.append({
                    "type": event_type,
                    "tool_name": tool_name,
            ...
            reply_stream_result = await bridge.run_reply_stream(
                bi_agent, msg=msg, session=session, on_tool_call=_collect_tool_event,
            )
