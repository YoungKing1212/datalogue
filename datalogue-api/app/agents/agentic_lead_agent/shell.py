...
# File Name   : shell.py
# Description:
#   Datalogue AgenticLeadAgent-first AS-R0 契约层。
#
# Responsibilities:
#   - 定义 AgenticLeadAgent 统一受控运行环境的 Agent Registry、工具白名单和回合契约。
...
class DatalogueAgenticShell:
    ...
    def prepare_turn(
        ...
        *,
        ...
        log_lifecycle(
            "agentic_lead_agent.io.input",
            question_length=len(question or ""),
            context_keys=sorted(
                key for key in raw_context if not self._is_forbidden_context_key(str(key))
            ),
        )
        # 调试开关打开时保留原始输入，便于确认上层到底把哪些字段交给了 LeadAgent。
        log_raw("agentic_lead_agent.raw.input", question=question, context=raw_context)
        ...
        status: AgenticShellStatus = (
            "ready"
            if selected_agent == "bi_agent" or selected_agent in self.enabled_optional_agents
            else "disabled"
        )
        ...
        projected_context = self.project_context(question=question, context=raw_context)
        ...
        decision_reason = (
            "bi_agent_enabled_for_query"
            if status == "ready" and selected_agent == "bi_agent"
            else "agent_disabled_or_unsupported"
        )
        log_lifecycle(
            "agentic_lead_agent.reasoning.decision",
            task_type=task_type,
            selected_subagent=selected_agent,
            status=status,
            decision_reason=decision_reason,
            allowed_capabilities=business_capabilities,
            allowed_tools=allowed_tools,
            projected_context_keys=sorted(projected_context.model_dump().keys()),
        )
        log_lifecycle(
            "agentic_lead_agent.io.output",
            task_type=task_type,
            selected_subagent=selected_agent,
            status=status,
            allowed_capabilities=business_capabilities,
            projected_context=projected_context.model_dump(),
        )
        # raw output 只服务本地调试，完整展示本轮路由契约，方便排查白名单和投影问题。
        log_raw(
            "agentic_lead_agent.raw.output",
            status=status,
            task_type=task_type,
            selected_agent=selected_agent,
    ...
    def route_action_from_contract(
        ...
        *,
        ...
        if requested_capability not in allowed_capabilities:
            # 即使选中 BI Agent，也只能路由 query_dataset/query_multiple_datasets 两个业务能力。
            return AgenticShellAction(
                action_type=f"{contract.selected_agent}.disabled",
                status="disabled",
                selected_agent=contract.selected_agent,
...
# 正式目标命名；迁移期保留 DatalogueAgenticShell 作为旧调用方兼容名。
