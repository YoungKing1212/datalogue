# ============================================================
# File Name   : boundary.py
# Description:
#   AgentScope Runtime 接入前的 Datalogue 边界适配契约。
#
# Responsibilities:
#   - 将 AgenticLeadAgent 的受控回合契约转换为 Runtime 可见的安全契约。
...
from app.agents.agentic_lead_agent import (
...
class DatalogueAgentScopeRuntimeDriver:
    ...
    def __init__(self, *, shell: AgenticLeadAgent | None = None) -> None:
        self.shell = shell or AgenticLeadAgent()
    ...
    def prepare_runtime(
        ...
        *,
        ...
        # Runtime boundary 必须显式携带 Agent action，避免后续运行层自行放宽工具能力。
