...
def _project_direct_query_stream_event(
    event: dict[str, Any],
    *,
    ...
    if conversation is not None:
        # 新建草稿首轮发送后，把真实会话 ID 回传给前端，避免本地 thread 永久悬空。
        public_event["conversation_id"] = conversation.id
        public_event["title"] = conversation.title
    return public_event
...
def _ensure_direct_stream_conversation(
    db: Session,
    payload: AgenticDirectQueryRequest,
) -> Conversation | None:
    """直连流式首轮没有 conversation_id 时，创建可删除、可回放的真实会话。"""

    if isinstance(payload.conversation_id, int):
        return db.get(Conversation, payload.conversation_id)
    title = (payload.question or "").strip()[:80] or "新对话"
    ...
    db.add(conversation)
    db.flush()  # runner 和消息持久化需要同一事务内拿到真实 conversation.id。
    return conversation
...
async def stream_agentic_direct_query(
    ...
    async def event_stream():
        ...
        conversation = _ensure_direct_stream_conversation(db, payload)
        runner_payload = (
            payload.model_copy(update={"conversation_id": conversation.id})
            if conversation is not None
            else payload
        )
        if payload.conversation_id is None and conversation is not None:
            should_commit = True
        runner = AgenticDirectQueryRunner(db=db)
        async for event in runner.stream(**_runner_kwargs(runner_payload)):
            public_event = _project_direct_query_stream_event(event, conversation=conversation)
            if not user_message_persisted:
                # runner 已经完成历史摘要读取后再写本轮问题，避免重复追问同一句时误跳过历史里的同文本问题。
                user_message_persisted = True
                if _persist_direct_user_message(db, runner_payload):
                    should_commit = True
            if public_event.get("type") == "final":
                if _persist_direct_assistant_message(db, payload=runner_payload, public_event=public_event):
                    should_commit = True
                if public_event.get("artifact_ref"):
                    # 直连流式接口也必须在 final 前确保 artifact 事务可见。
                    should_commit = True
                if should_commit:
                    db.commit()  # final 下发前提交，前端随后的 thread list reload 才能看到真实会话。
                    should_commit = False
