...
async def test_direct_query_runner_streams_agentscope_messages_and_final_result(db_session):
    ...
    assert events[2]["from_agent"] == "agentic_lead_agent"
    assert events[2]["to_agent"] == "bi_agent"
    assert events[3]["agent"] == "bi_agent"
    assert events[4]["role"] == "user"
    assert events[5]["role"] == "assistant"
    assert events[7]["agent"] == "agentic_lead_agent"
    assert events[7]["phase"] == "final_prompt"
    assert events[8]["agent"] == "agentic_lead_agent"
    assert events[8]["phase"] == "final_response"
...
def test_agentic_lead_agent_direct_query_stream_api_creates_conversation_for_draft(
    monkeypatch,
    client,
    db_session,
):
    ...
    with client.stream(
        ...
        json={
            "question": "统计合同总金额",
            "dataset_id": 12,
            "trace_id": "trace-direct-stream-draft",
        },
    ) as response:
        lines = [line for line in response.iter_lines() if line.startswith("data:")]
    ...
    assert captured_kwargs["conversation_id"] == conversation_id
    assert payload["title"] == "统计合同总金额"

    conversation = db_session.get(Conversation, conversation_id)
    assert conversation is not None
    assert conversation.title == "统计合同总金额"
    assert conversation.dataset_id == 12
    messages = (
        db_session.query(Message)
