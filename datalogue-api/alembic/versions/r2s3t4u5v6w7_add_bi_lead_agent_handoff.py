...
def upgrade() -> None:
    if not _has_table("bi_lead_agent_run"):
        op.create_table(
            ...
            sa.Column("phase", sa.String(length=60), server_default="created", nullable=False, comment="当前编排阶段。"),
            ...
            sa.Column("trace_id", sa.String(length=120), nullable=True, comment="观测 trace ID。"),
            ...
            sa.UniqueConstraint("trace_id", name="uq_bi_lead_agent_run_trace_id"),
    ...
    if not _has_table("bi_lead_agent_confirmation"):
        op.create_table(
            ...
            sa.Column("dataset_id", sa.Integer(), nullable=True, comment="确认后选择的数据集 ID。"),
            sa.Column("confirmed_question", sa.Text(), nullable=False, comment="用户确认后的问题文本。"),
            sa.Column("task_goal", sa.Text(), nullable=True, comment="交给 DatasetAgent 的任务目标。"),
            sa.Column(
            ...
            sa.Column("routing_rationale", sa.Text(), nullable=True, comment="LeadAgent 路由理由。"),
            ...
            sa.Column("trace_id", sa.String(length=120), nullable=True, comment="观测 trace ID。"),
            sa.Column("parent_run_id", sa.String(length=120), nullable=True, comment="上游 run 标识，用于跨层回放。"),
    ...
    if not _has_table("bi_agent_handoff"):
        op.create_table(
            ...
            sa.Column("child_agent", sa.String(length=80), nullable=False, comment="子 Agent 名称。"),
            sa.Column("child_run_id", sa.String(length=120), nullable=True, comment="子 Agent run ID。"),
            sa.Column("dataset_id", sa.Integer(), nullable=True, comment="移交目标数据集 ID。"),
            sa.Column("task_id", sa.String(length=120), nullable=True, comment="业务任务 ID。"),
            sa.Column("trace_id", sa.String(length=120), nullable=True, comment="观测 trace ID。"),
    ...
    for table_name, index_name, columns, unique in [
        ...
        ("bi_lead_agent_run", op.f("ix_bi_lead_agent_run_trace_id"), ["trace_id"], True),
