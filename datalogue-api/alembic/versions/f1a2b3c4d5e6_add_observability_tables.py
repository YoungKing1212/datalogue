...
def upgrade() -> None:
    ...
    if not inspector.has_table("observability_trace_index"):
        op.create_table(
            ...
            sa.Column("langfuse_trace_id", sa.String(length=120), nullable=False, comment='Langfuse Trace ID'),
            sa.Column("langfuse_session_id", sa.String(length=120), nullable=False, comment='Langfuse Session ID'),
            sa.Column("conversation_id", sa.Integer(), nullable=True, comment='关联的会话 ID'),
            sa.Column("message_id", sa.Integer(), nullable=True, comment='关联的消息 ID'),
            sa.Column("dataset_id", sa.Integer(), nullable=True, comment='关联的数据集 ID'),
            sa.Column("entry_route", sa.String(length=60), nullable=True, comment='问数入口路由类型'),
            sa.Column("status", sa.String(length=30), server_default="success", nullable=False, comment='执行状态（success/error等）'),
            sa.Column("total_tokens", sa.Integer(), server_default="0", nullable=False, comment='本次请求总 token 消耗'),
            sa.Column("total_cost", sa.Float(), server_default="0", nullable=False, comment='本次请求总费用'),
    ...
    if not inspector.has_table("trace_annotation_candidate"):
        op.create_table(
            ...
            sa.Column("message_id", sa.Integer(), nullable=True, comment='关联的消息 ID'),
            sa.Column("dataset_id", sa.Integer(), nullable=True, comment='关联的数据集 ID'),
            sa.Column("reason", sa.String(length=100), nullable=False, comment='进入候选池的原因（thumbs_down/error/low_quality等）'),
            sa.Column("status", sa.String(length=30), server_default="pending", nullable=False, comment='处理状态（pending/annotated/dismissed）'),
            sa.Column("payload", sa.JSON(), nullable=True, comment='附加数据载体'),
            sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), comment='创建时间'),
            sa.ForeignKeyConstraint(["conversation_id"], ["conversation.id"]),
            sa.ForeignKeyConstraint(["dataset_id"], ["semantic_dataset.id"]),
            sa.ForeignKeyConstraint(["message_id"], ["message.id"]),
            sa.PrimaryKeyConstraint("id"),
            comment='Trace 人工标注候选池表',
        )
    for index_name, columns in {
